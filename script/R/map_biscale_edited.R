...
nosp <- read_rds("ouput/suhi_2023_04_23.rds") %>% 
  select(IDMANZANA,MHI_summer)

spatial <- read_rds("ouput/suhi_final_data_index.rds") %>% 
  left_join(nosp,by = "IDMANZANA")

data <- spatial %>% 
  drop_na(Index)

sp <- st_read("raw_data/gpkg/blocks_lima.gpkg") %>% select(IDMANZANA) %>% 
  inner_join(data,by = "IDMANZANA") %>% 
  drop_na(MHI_summer)
...
ggsave(
  filename = "ouput/png/MHI_biscalemap_summer_v3.png",
  plot = end,
  width = 14,
  height = 8,
  bg = "white"
...
ggplot(data = data,aes(y = SUHI,x= MHI_summer)) + 
  geom_point() + 
